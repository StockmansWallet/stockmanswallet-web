// API route: returns fully-rendered Asset Register HTML for PDF generation.
// Accepts JWT via Authorization header. Used by the iOS app to generate
// identical PDFs using WKWebView as an HTML-to-PDF renderer.
// Debug: Builds HTML string directly (no JSX/renderToStaticMarkup) to avoid
// Turbopack restrictions on react-dom/server in route handlers.
...
// -- HTML Builder (mirrors report-template.tsx) --
function buildReportHTML(data: ReportData, movement: PortfolioMovementSummary | null): string {
...
<section class="mt-5 break-avoid">
  ...
  <div class="grid-4 card mb-3" style="padding:12px 20px">
    ...
    <div><p class="label">Head Count</p><p class="val" style="font-size:13px">${m.openingHeadCount} → ${m.closingHeadCount}</p></div>
  </div>
...
function buildHerdCardHTML(h: HerdReportData): string {
  ...
  return `
  <div class="card break-avoid" style="margin-bottom:6px">
