...
export const metadata = { title: "Client Herd Details" };
...
export default async function AdvisorHerdDetailPage({
  params,
}: {
  params: Promise<{ id: string; herdId: string }>;
}) {
  const { id: connectionId, herdId } = await params;
  ...
  const { data: allPrices } = await serviceClient.rpc("latest_saleyard_prices", {
  ...
  const dataAgeDays = valuation.dataDate ? Math.floor((Date.now() - new Date(valuation.dataDate).getTime()) / 86400000) : 0;
  ...
  if (herd.initial_weight > 0 && herd.daily_weight_gain > 0) {
    ...
    const now = new Date();
    projectedWeight = calculateProjectedWeight(herd.initial_weight, created, herd.dwg_change_date ? new Date(herd.dwg_change_date) : now, now, herd.previous_dwg ?? herd.daily_weight_gain, herd.daily_weight_gain);
  }
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        <span className="text-xs text-text-muted">Client herd data. Advisor overrides below.</span>
      </div>
          ...
          {herdValue > 0 && (
            <div className="rounded-2xl bg-white/5 p-5">
              <div className="flex items-center gap-3">
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${isFallback ? "bg-red-500/15" : isStale ? "bg-amber-500/15" : "bg-brand/15"}`}>
                  {isFallback ? <AlertTriangle className="h-5 w-5 text-red-400" /> : isStale ? <Clock className="h-5 w-5 text-amber-400" /> : <DollarSign className="h-5 w-5 text-brand" />}
                </div>
                <div>
