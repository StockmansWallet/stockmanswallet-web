...
export function ClientReportTab({ reportData, clientName }: ClientReportTabProps) {
  if (!reportData) {
    return (
      <Card>
  ...
  const { herdData, salesData, totalValue, totalSales, herdComposition, dateRange } = reportData;
  const totalHead = herdData.reduce((s, h) => s + h.headCount, 0);
  const totalSalesHead = salesData.reduce((s, r) => s + r.headCount, 0);
  const dateRangeStr = `${fmtDate(dateRange.start)} - ${fmtDate(dateRange.end)}`;
  ...
  if (isEmpty) {
    return (
      <Card>
        <CardContent className="py-8 text-center">
          <p className="text-sm text-text-muted">No report data for this period.</p>
        </CardContent>
      </Card>
    );
  }
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs text-text-muted">
                Financial Summary, {dateRangeStr}
              </p>
              <p className="text-xs text-text-muted">
                Generated {new Date().toLocaleDateString("en-AU", { day: "2-digit", month: "long", year: "numeric" })}
              </p>
            </div>
            <Badge className="bg-purple-500/15 text-purple-400">Read-only</Badge>
          </div>
        </CardContent>
      </Card>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs tabular-nums text-text-muted">
                {herdData.length} herds, {fmt(totalValue)}
              </span>
            </div>
          </CardHeader>
                ...
                <tbody className="divide-y divide-white/5">
                  {herdData.map((h) => (
                    <tr key={h.id} className="transition-colors hover:bg-white/[0.02]">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-text-secondary">{h.weight.toFixed(0)} kg</td>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs tabular-nums text-text-muted">
                {salesData.length} sales, {fmt(totalSales)}
              </span>
            </div>
          </CardHeader>
          <CardContent className="px-0 pb-0">
