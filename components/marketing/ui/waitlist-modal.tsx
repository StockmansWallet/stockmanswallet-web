...
export function WaitlistModal({ open, onClose }: WaitlistModalProps) {
      ...
      >
        <div className="flex flex-col items-center py-8 text-center">
          <div className="bg-success/15 flex h-14 w-14 items-center justify-center rounded-full">
            <Check className="text-success h-7 w-7" aria-hidden="true" />
          </div>
          ...
          <button
            onClick={handleClose}
            className="bg-brand hover:bg-brand-light mt-7 inline-flex h-10 cursor-pointer items-center justify-center rounded-xl px-6 text-sm font-semibold text-white transition-all duration-150 active:scale-[0.97]"
          >
            Done
          </button>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        <aside className="flex flex-col justify-between border-b border-white/[0.08] bg-white/[0.045] p-6 md:border-r md:border-b-0 md:p-8">
          <div>
            ...
            <p className="text-text-secondary mt-3 text-sm leading-relaxed">
              The landing page is live now. The app launches June 2026 on the App Store, with early
              access opened in waves.
            </p>
          </div>
          ...
          <div className="mt-8 space-y-3">
            {STEPS.map((item, index) => {
              ...
              const complete = index === 0 ? contactValid : index === 1 ? hasDetails : hasFeatures;
        ...
        <form onSubmit={handleSubmit} className="flex min-h-[520px] flex-col bg-white/[0.025]">
          <div className="flex-1 overflow-y-auto p-6 md:p-8">
            <p className="text-brand text-xs font-semibold tracking-wider uppercase">
              {activeStep.eyebrow}
            </p>
            <h3 className="mt-2 text-xl font-semibold text-white">{activeStep.title}</h3>
            <p className="text-text-secondary mt-1 text-sm">{activeStep.description}</p>
                ...
                >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label
                        htmlFor="wl-name"
                        className="text-text-muted mb-1.5 block text-xs font-medium tracking-wider uppercase"
                      >
                      ...
                      <AnimatePresence>
                        {showEmailHint && (
                          <motion.p
                            ...
                            exit={{ opacity: 0, y: -4 }}
                            className="text-error/80 mt-1.5 text-xs"
                          >
                ...
                >
                  ...
                  <label className="group flex cursor-pointer items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.025] p-4">
                    <input
                      ...
                      className="accent-brand h-4 w-4 shrink-0 cursor-pointer rounded border-white/20 bg-transparent"
                    />
                    <span className="text-text-muted group-hover:text-text-secondary text-sm transition-colors">
                      Keep me updated with product news and launch updates
                    </span>
                  </label>

                  {status === "error" && <p className="text-error text-xs">{errorMsg}</p>}
                </motion.div>
              )}
            </AnimatePresence>
          </div>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {step < STEPS.length - 1 ? (
                ...
                <button
                  ...
                  className="bg-brand hover:bg-brand-light inline-flex h-11 min-w-32 cursor-pointer items-center justify-center rounded-full px-5 text-sm font-semibold text-white transition-colors disabled:pointer-events-none disabled:opacity-40"
                >
                  {status === "submitting" ? (
